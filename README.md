...
一个美观的**个人导航页面**，支持动态背景、一言 API、深浅色模式切换，并带有**完整的后台管理功能**。
...
1. 访问 /admin.html，例如 https://nav.fnosi.top/admin.html
2. 使用设置的管理员账户登录
3. 可以添加、编辑、删除导航链接
4. 保存后自动同步到 Cloudflare KV，前台实时更新
...
2. 使用强 JWT 密钥：JWT_SECRET 使用至少 32 位随机字符串
3. 限制管理页面访问：可在 Cloudflare Dashboard 中设置 IP 访问规则
4. 定期更新依赖：保持 Wrangler CLI 为最新版本
...
可以直接复制到 GitHub 仓库的 `README.md` 文件中。
